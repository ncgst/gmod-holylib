...
- Required delivery keeps those replies inside GMod's original synchronous file-request callback. Before `SendServerInfo`, HolyLib computes the complete worst-case canonical-stub burst from the pinned generation and verifies the client's engine netchannel capacity. Existing capacity is left untouched when the burst plus 64 KiB of sign-on headroom already fits; otherwise the grown buffer preserves the channel's entire previous capacity as headroom. A resize with a non-empty scratch stream, missing stub, arithmetic overflow, or engine capacity clamp fails the baseline closed. LuaPack does not replay the sender later, write private fragment queues, or change native/opt-out lanes.

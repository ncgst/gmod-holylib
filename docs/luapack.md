...
- Canonical stub requests are deduplicated per physical slot and paced every 50 ms with 16 KiB per-client, 128 KiB server-wide staging, and 32-batch server-wide budgets while retaining 64 KiB of reliable-stream headroom. LuaPack starts one batch per client when no earlier LuaPack batch remains in the reliable scratch stream and submits every body through GMod's exact engine-owned custom-message symbol. The exact void call is the ownership boundary: if it retains bytes in the observed reliable scratch stream, LuaPack waits for them to drain before staging another batch; if the engine transfers them immediately, no scratch-growth proof is required. LuaPack never invokes transport through the locally mirrored `IVEngineServer` or `INetChannel` ABI and never writes the SDK mirror's private fragment list. An invalid payload, required ordered-hash failure, rejected exact dispatch, reliable overflow, or retained scratch bytes lasting 60 seconds is fatal instead of leaving the client in Requesting Lua. PRESPAWN waits for queued and engine-owned stub work. A new `SendServerInfo` baseline, physical disconnect, kill switch, or level/module shutdown clears unsent work; late game-layer callbacks do not.

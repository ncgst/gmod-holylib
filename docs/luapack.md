...
- Required delivery keeps those replies inside GMod's original synchronous file-request callback. Before `SendServerInfo`, HolyLib computes the complete worst-case canonical-stub burst from the pinned generation and verifies the client's engine netchannel capacity. Existing capacity is left untouched when the burst plus 64 KiB of sign-on headroom already fits; otherwise HolyLib requests enough growth to preserve the channel's entire previous capacity, while accepting an engine clamp only if the burst still retains at least 64 KiB. A resize with a non-empty scratch stream, missing stub, arithmetic overflow, or clamp below that minimum fails the baseline closed. LuaPack does not replay the sender later, write private fragment queues, or change native/opt-out lanes.

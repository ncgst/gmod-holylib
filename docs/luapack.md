...
- Canonical stub requests are deduplicated per physical slot and paced every 50 ms with 16 KiB per-client, 128 KiB server-wide staging, and 32-batch server-wide budgets while retaining 64 KiB of reliable-stream headroom. LuaPack starts one batch per client when no earlier LuaPack batch remains in the reliable scratch stream and submits every body through GMod's exact engine-owned custom-message path. It verifies that the engine appended the preflighted batch, then waits for the observed scratch stream to drain into the engine network path before staging another batch; it never invokes transport through the locally mirrored `INetChannel` ABI or writes the SDK mirror's private fragment list. An invalid payload, required ordered-hash failure, failed engine append, reliable overflow, or scratch bytes retained for 60 seconds is fatal instead of leaving the client in Requesting Lua. PRESPAWN waits for queued and engine-owned stub work. A new `SendServerInfo` baseline, physical disconnect, kill switch, or level/module shutdown clears unsent work; late game-layer callbacks do not.

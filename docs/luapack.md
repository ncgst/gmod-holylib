...
- Canonical stub requests are deduplicated per physical slot and paced every 50 ms with 16 KiB per-client, 128 KiB server-wide staging, and 32-batch server-wide budgets while retaining 64 KiB of reliable-stream headroom. LuaPack starts one batch per client when no earlier LuaPack batch remains in the reliable scratch stream and submits every body through the resolved engine implementation of `CNetChan::SendNetMsg` on the client's authoritative channel. Each call must report success and append to the observed reliable scratch stream; LuaPack then waits for that bounded batch to drain before staging another. It never replays `CVEngineServer::GMOD_SendToClient` from the deferred scheduler, invokes transport through a locally mirrored client/channel vtable, or writes the SDK mirror's private fragment list. An invalid payload, required ordered-hash failure, rejected exact append, reliable overflow, or retained scratch bytes lasting 60 seconds is fatal instead of leaving the client in Requesting Lua. PRESPAWN waits for queued and engine-owned stub work. A new `SendServerInfo` baseline, physical disconnect, kill switch, or level/module shutdown clears unsent work; late game-layer callbacks do not.

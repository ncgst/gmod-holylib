...
Active hash updates reserve the final RequestLuaFiles rescan envelope as well as their own wire record. The rescan keeps its eight-bit payload and is counted only after its complete 34-bit record has entered the reliable stream. If another writer consumes the reserved space, one coalesced rescan remains pending for that client, channel, user ID, and connection challenge. Retries run before new active hash work even when the file queue is empty; disconnect, replacement connection, level shutdown, or feature disable discards stale work.

...
- Required delivery records each baseline-owned request inside GMod's original file-request callback, but does not append the complete cold response burst there. Before `SendServerInfo`, HolyLib verifies reliable capacity for one bounded frame of canonical placeholders plus 64 KiB of sign-on headroom. The same baseline pass retains an O(1) per-slot set of the file IDs it advertised as canonical. An unchanged requested ID can therefore enter a deduplicated queue without repeating Lua-cache lookup, path normalization, registry locking, or base/current identity comparison; any source or registration-mode transition invalidates that ID before delivery. Main-thread `Think` then appends the exact `svc_GMod_ServerToClient`/`LuaFileDownload` wire record in fair round-robin client order, subject to the global frame budget and available reliable space. PRESPAWN remains gated until the queue is empty. A resize with a non-empty scratch stream, missing stub, arithmetic overflow, lost pinned identity/channel, or clamp below the minimum fails closed. LuaPack does not use private fragment queues or change native/opt-out lanes.
...
| `holylib_gmoddatapack_luapack_required_stub_budget` | `32` | Global maximum canonical required placeholders staged into reliable netchannels per server frame. The fair queue prevents a cold client's batched Lua request from monopolizing `CNetChan::ProcessMessages`. |
...
The client may request every cold canonical ID in one network message. HolyLib records those already-baselined IDs in O(1), coalesces duplicate requests, and stages at most `holylib_gmoddatapack_luapack_required_stub_budget` placeholders globally per server frame in round-robin client order. PRESPAWN remains gated until that client's queue is empty. Insufficient reliable scratch space waits for a later frame; a lost lane, hook, payload, connection, or file identity fails closed instead of switching the join to native.

...
| `holylib_gmoddatapack_luapack_registration_refresh_budget` | `64` | Maximum registered Lua paths reprocessed per frame after an enable/hook-capability transition. Pending initial baselines remain queued until the bounded refresh publishes a coherent identity set. Values are clamped to `1..1024`. |

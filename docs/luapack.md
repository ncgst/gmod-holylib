...
- Exact pack lookup keys come from each registered string-table path, not `LuaFile::source` (which is registration provenance and can be shared by many files). Duplicate exact keys invalidate a pack during both server build validation and client parsing instead of silently executing the wrong source.

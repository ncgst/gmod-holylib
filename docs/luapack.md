...
- Canonical stub requests are deduplicated per physical slot and paced every 50 ms with 16 KiB per-client and 128 KiB server-wide staging budgets while retaining 64 KiB of reliable-stream headroom. Only one batch per client is transferred at a time into the netchannel's owned reliable-fragment list, so a later scratch reset cannot discard it and owned backlog remains bounded. While those fragments remain, LuaPack emits at most one packet per client per 50 ms and only after that netchannel's clear-time rate window opens; this avoids Source's one-packet-per-second PRESPAWN cadence without bypassing the configured client rate. The pump rotates across slots and caps each 50 ms pass at 32 packets to bound a connection flood's synchronous work. A transient staging rejection defers the same request without consuming it; only an invalid payload or required ordered-hash failure is fatal. PRESPAWN waits for queued and owned stub work. A new `SendServerInfo` baseline, physical disconnect, kill switch, or level/module shutdown clears unsent work; late game-layer callbacks do not.

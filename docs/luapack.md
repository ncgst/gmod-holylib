...
- Source suppresses byte-identical canonical hash publications. For active clients, LuaPack appends the current per-client hash and a complete native filename/body refresh together. The client installs the SHA-256-prefixed LZMA body, invalidates its Lua cache, and enters the normal engine reload handler; no file request or opcode-3 rescan is required. Exact restoration uses the pinned canonical placeholder body. Explicit disk capture repairs the existing engine source and invalidates its compressed length/cursor before publication. It accepts only a safe existing registration with an engine source object. Unchanged explicit recovery forces another complete transaction and returns `true, "refresh_queued"`; unchanged automatic paths do nothing. The shared watcher ingress preserves server-side reload, routes its active recipients through this same queue to avoid double execution, and leaves joining recipients on native delivery. Unrelated GMod messages and unknown registrations retain their original path.
- `includes/init.lua` is always real because it installs the resolver. It is never read from the pack.
- Failed automatic disk reads retain the last captured source and queue at most four main-thread rereads, delayed by 0.25, 0.5, 1, and 2 seconds, with a five-second lifetime. Duplicate watcher notifications coalesce without extending that lifetime or resetting attempts. The queue holds at most 256 registrations and attempts at most eight reads per frame. Full `addons/`, `gamemodes/`, and `lua/` registrations are read through their exact `MOD` paths; only Lua-relative registrations without an exact source path use the mounted `GAME` lookup. This avoids malformed `lua/addons/...` paths and reading another addon's shadowing source. Retries do not replay the engine watcher or force unchanged bytes through active refresh. Success, lost registration identity, Lua/map shutdown, and feature disable retire pending work; exhaustion is logged. Explicit `RefreshExistingLuaFile` remains synchronous and returns `unreadable` without scheduling a disk reread.
...
Before Source serializes `client_lua_files`, a required connection receives canonical hashes only for paths whose current SHA-256 still equals the base. Init, hotfixes, and late registrations receive real source hashes. An active native refresh appends that file's current hash and matching body in reliable order; exact restoration appends the canonical hash and placeholder body. Bookkeeping advances only after the complete transaction has entered the stream. This records successful staging, not a client execution acknowledgement.
...
The all-file registration transition is independently bounded by `holylib_gmoddatapack_luapack_registration_refresh_budget`. The same global budget caps active-client refresh attempts when a canonical publication is byte-identical and Source therefore emits no update. Each attempt preflights the exact `svc_UpdateStringTable` record plus the entire variable-sized filename/body message against the recipient's engine-owned reliable stream. Deferred attempts count against the limit exactly like successful writes. Budget exhaustion resumes from the next unprocessed slot, while a completed pass with failed recipients retains only those failures and waits briefly before another pass. This prevents a parked or backpressured channel from causing an unbounded per-frame retry loop. The per-client native SHA-256 identities used by `SendServerInfo` run on the existing compression worker, while each frame publishes only its bounded registration slice. Missing sources remain fail-closed and are retried without rescanning the completed corpus; baseline service also waits for that worker-owned native hash cache. Map-base construction stays held until the transition completes, and required-mode `SendServerInfo` baselines remain queued through successful initial base publication, so a cold boot or kill-switch transition cannot expose a mixed native/canonical string table, move full-corpus baseline hashing back onto a joining client, or reject an admitted join merely because the immutable base is still building.

Active refresh also has a global 256 KiB wire budget per frame and a 64 KiB native payload ceiling, including opcode, filename, terminating NUL, byte count, and body. The full transaction must fit before either record is written; no hash-only partial transaction is left behind on a capacity retry. Explicit native bodies are compressed on the existing worker and retained in HolyLib-owned storage, with stale results discarded after a source change. The ordinary watcher supplies its already-compressed body. Retry ownership captures client, channel, user ID, and challenge at publication, so a replacement connection cannot inherit a forced refresh. Permanent invalid/oversized transactions fail closed for the affected recipient; temporary capacity shortages remain paced and budgeted. Feature disable and level shutdown discard queued work.
...
The earlier September 10 client test established that opcode 3 is ignored. Active refresh now uses the complete native filename/body transaction described in [the receiver evidence](luapack-gluapack-re.md#15-native-filenamebody-refresh-contract). Source tests and successful transport staging alone do not establish runtime acceptance; retain the gates below until their recorded controlled-client observations pass.
...
10. Restart the isolated client with `+tv_nochat no_gluapack`. Confirm native Lua for the whole connection and rejection of late lane changes. For a changed existing fixture, block only its ordinary watcher and call `gmoddatapack.RefreshExistingLuaFile("path/to/file.lua")`. Require `true, "captured"`, then a matching client cache body and exactly one automatic execution of the new source. Unchanged explicit recovery returns `true, "refresh_queued"`. Follow a native H1 -> H2 -> H1 restoration without forced recovery, then repeat on a required connection and verify canonical restoration. Unblock the watcher and require exactly one execution for an ordinary edit as well. Prime a real nonempty engine compression allocation through the original native sender before another explicit change; verify retained pointer/capacity, zero written length/cursor, correct native recompression, and unchanged reuse. A staged-transaction log alone is insufficient.
...
The per-client hash update uses GMod's variable-size string-table userdata format: a 19-bit byte count of 32 precedes the SHA-256 bytes. A currently published 32-byte hash does not imply a fixed-size table. Omitting the byte count makes the receiver consume the hash prefix as a length and replace the client entry with invalid userdata. Capacity accounting includes this field and reserves the complete following filename/body refresh. See the engine-parser evidence in [the interoperability notes](luapack-gluapack-re.md#13-active-string-table-userdata-length--confirmed-for-the-inspected-engines).

...
- Canonical stub requests are deduplicated per physical slot and paced every 50 ms with 16 KiB per-client and 128 KiB server-wide staging budgets while retaining 64 KiB of reliable-stream headroom. Accepted batches move from the bounded scratch stream into the netchannel's owned reliable-fragment list without forcing a packet, so normal engine bandwidth pacing applies and a later scratch reset cannot discard them. A transient staging rejection defers the same request without consuming it; only an invalid payload or required ordered-hash failure is fatal. PRESPAWN waits for the pending stub queue. A new `SendServerInfo` baseline, physical disconnect, kill switch, or level/module shutdown clears unsent work; late game-layer callbacks do not.

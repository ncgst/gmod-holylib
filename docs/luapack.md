...
- Required delivery keeps those replies inside GMod's original synchronous file-request callback. Before `SendServerInfo`, HolyLib computes the complete worst-case canonical-stub burst from the pinned generation and verifies the client's engine netchannel capacity. Existing capacity is left untouched when the burst plus 64 KiB of sign-on headroom already fits; otherwise HolyLib requests enough growth to preserve the channel's entire previous capacity, while accepting an engine clamp only if the burst still retains at least 64 KiB. The same baseline pass retains an O(1) per-slot set of the file IDs it advertised as canonical. An unchanged requested ID can therefore reuse that exact decision without repeating Lua-cache lookup, path normalization, registry locking, or base/current identity comparison; any source or registration-mode transition invalidates that ID before it can use the fast path. Each requested canonical stub is then appended as the same `svc_GMod_ServerToClient`/`LuaFileDownload` wire record that the engine sender writes, directly to that already-verified reliable stream. This keeps the engine's request ownership and ordering while avoiding both repeated policy resolution and thousands of repeated `CVEngineServer::GMOD_SendToClient` dispatches in one `CNetChan::ProcessMessages` call. A resize with a non-empty scratch stream, missing stub, arithmetic overflow, insufficient append space, or clamp below the minimum fails closed. LuaPack does not replay the sender later, write private fragment queues, or change native/opt-out lanes.

...
- Canonical stub requests are deduplicated per physical slot and paced every 50 ms with 16 KiB per-client, 128 KiB server-wide staging, and 32-transmit server-wide budgets while retaining 64 KiB of reliable-stream headroom. LuaPack starts one batch per client only when the engine reports no earlier reliable work, stages it through the engine netchannel, and calls only virtual transmit/state boundaries so the engine owns and advances its private fragments; LuaPack never writes the SDK mirror's private fragment list. The next batch waits for the engine's virtual `HasPendingReliableData` result to clear. A transient staging rejection defers the same request without consuming it; an invalid payload, required ordered-hash failure, failed engine-ownership boundary, reliable overflow, or batch still pending after 60 seconds is fatal instead of leaving the client in Requesting Lua. PRESPAWN waits for queued and engine-owned stub work. A new `SendServerInfo` baseline, physical disconnect, kill switch, or level/module shutdown clears unsent work; late game-layer callbacks do not.

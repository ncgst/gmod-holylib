...
- Canonical stub requests are deduplicated per physical slot and paced every 50 ms with 16 KiB per-client and 128 KiB server-wide staging budgets while retaining 64 KiB of reliable-stream headroom. A bandwidth-choked channel or transient staging rejection defers the same request without consuming it; only an invalid payload or required ordered-hash failure is fatal. PRESPAWN waits for the pending stub queue, and hash/body state is committed only after the batch has moved into reliable fragments. A new `SendServerInfo` baseline, physical disconnect, kill switch, or level/module shutdown clears unsent work; late game-layer callbacks do not.

...
static bool SendCompressedLuaFile(int clientIdx, int fileID, const Bootil::AutoBuffer& compressed,
	const void* expectedHash = nullptr, int expectedHashLength = -1)
{
	static constexpr int LUA_FILE_HASH_BYTES = 32;
	const bool substituteHash = expectedHashLength >= 0;
	if (substituteHash && (!expectedHash || expectedHashLength != LUA_FILE_HASH_BYTES || compressed.GetWritten() < LUA_FILE_HASH_BYTES))
		return false;
	...
	if (substituteHash)
	{
		// The client compares the transfer's leading SHA-256 with client_lua_files userdata.
		// A substituted body must therefore retain that registered identity even though the
		// LZMA stream contains a tiny resolver stub. The body itself is validated by LuaPack's
		// immutable generation MD5 before the resolver compiles the packed source.
		msg.WriteBytes(expectedHash, LUA_FILE_HASH_BYTES);
		msg.WriteBytes(compressed.GetBase(LUA_FILE_HASH_BYTES), compressed.GetWritten() - LUA_FILE_HASH_BYTES);
	}
	else
	{
...
static void hook_GModDataPack_SendFileToClient(GModDataPack* pDataPack, int clientIdx, int fileID)
{
	...
	if (delivery.action == HolyLib::LuaPack::DeliveryAction::Stub && delivery.compressed)
	{
		...
		// Preserve the globally registered source hash in the transfer header; using the stub's
		// own hash makes the client discard the otherwise valid substituted Lua body.
		int expectedHashLength = 0;
		const void* expectedHash = clientFiles->GetStringUserData(fileID, &expectedHashLength);
		if (!SendCompressedLuaFile(clientIdx, fileID, *delivery.compressed, expectedHash, expectedHashLength))
		{
			Warning(PROJECT_NAME " - luapack: cannot substitute fileID %i because client_lua_files has no 32-byte source hash\n", fileID);
			if (HolyLib::LuaPack::IsRequiredDeliveryClient(clientIdx))
				HolyLib::LuaPack::DisconnectRequiredClient(clientIdx, "the requested file has no canonical stringtable hash");
			else

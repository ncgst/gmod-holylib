...
void CNetChan::CompressFragments()
{
	...
	for ( int i=0; i<MAX_STREAMS; i++ )
	{
		...
		else // it's a file
		{
			...
			int compressedFileTime = g_pFullFileSystem->GetFileTime( compressedfilename );
			int fileTime = g_pFullFileSystem->GetFileTime( data->filename );

			if ( compressedFileTime >= fileTime )
			{
				// compressed file is newer than uncompressed file, use this one
				hZipFile = g_pFullFileSystem->Open( compressedfilename, "rb", NULL );
			}
			...
			else
			{
				...
				if ( COM_BufferToBufferCompress_LZ4( compressed, &compressedSize, uncompressed, uncompressedSize ) )
				{
					...
					if ( hZipFile != FILESYSTEM_INVALID_HANDLE )
					{
						...
						g_pFullFileSystem->Write( compressed, compressedSize, hZipFile );
						g_pFullFileSystem->Close( hZipFile );
			...
			if ( compressedFileSize > 0 )
			{
				...
				g_pFullFileSystem->Close( data->file );
				data->file = hZipFile;
				data->nUncompressedSize = data->bytes;
...
bool CNetChan::CreateFragmentsFromFile( const char *filename, int stream, unsigned int transferID )
{
	...
	if ( !g_pFullFileSystem->FileExists( filename, pPathID ) )
	{
...
bool CNetChan::HandleUpload( dataFragments_t *data, INetChannelHandler *MessageHandler )
{
	...
	else
	{
		...
		else
		{
			...
			if ( g_pFullFileSystem->FileExists( data->filename, pszPathID ) )
			{
			...
			else
			{
				...
				g_pFullFileSystem->CreateDirHierarchy( szParentDir, pszPathID );

				// Open new file for write binary.
				data->file = g_pFullFileSystem->Open( data->filename, "wb", pszPathID );
				...
				{
					g_pFullFileSystem->Write( data->buffer, data->bytes, data->file );
					g_pFullFileSystem->Close( data->file );

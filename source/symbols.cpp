...
namespace Symbols
{
	...
	const std::vector<Symbol> CBaseEntity_GMOD_ShouldPreventTransmitToPlayerSym = { //Find CBaseEntity::GetLuaEntity with "m_LuaEntity != ENTITY!"
		...
		Symbol::FromSignature("\x55\x31\xC0\x48\x89\xE5\x53\x48\x89\xFB\x48\x83\xEC\x18\x66"), // 55 31 C0 48 89 E5 53 48 89 FB 48 83 EC 18 66 (x64 260709; _ZTV11CBaseEntity slot 239 @ 0x91c6c0; prior sig read m_bPreventTransmit at +0x14E6, the member moved to +0x14EE)
	...
	const std::vector<Symbol> CBaseCombatCharacter_SetTransmitSym = {//Search for "models/manhack.mdl" and find CNPC_Manhack__Spawn, then vtable above
		...
		Symbol::FromSignature("\x55\x48\x89\xE5\x41\x57\x41\x56\x41\x55\x49\x89\xF5\x41\x54\x49\x89\xFC\x53\x48\x83\xEC\x18\x48\x8B\x47\x40\x48\x85\xC0\x0F\x84\x04"), // 55 48 89 E5 41 57 41 56 41 55 49 89 F5 41 54 49 89 FC 53 48 83 EC 18 48 8B 47 40 48 85 C0 0F 84 04 (x64 260709; _ZTV20CBaseCombatCharacter slot 21 @ 0x8f6810; prior sig had sub rsp,0x08 - frame grew to 0x18)
	...
	const std::vector<Symbol> CBaseAnimating_SetTransmitSym = {//Find "Setting CBaseAnimating to non-studio model %s  (type:%i)" to get CBaseAnimating__SetModel then find the last xref and take 4 function upper
		...
		Symbol::FromSignature("\x55\x48\x89\xE5\x41\x57\x41\x56\x41\x55\x49\x89\xF5\x41\x54\x53\x48\x89\xFB\x48\x83\xEC\x18\x48\x8B\x47\x40\x48\x85\xC0\x0F\x84\x1C"), // 55 48 89 E5 41 57 41 56 41 55 49 89 F5 41 54 53 48 89 FB 48 83 EC 18 48 8B 47 40 48 85 C0 0F 84 1C (x64 260709; _ZTV14CBaseAnimating slot 21 @ 0x8d65d0; prior sig pushed r14/r13/r12 and diverged at byte 5)

"""Inspect exact production GModDataPack::IsSingleplayer resolver behavior against caller-supplied ELF fixtures.

No engine instructions are executed. The fixture JSON is an array containing
file, sha256 and isp fields; isp is the hexadecimal ELF virtual address of the
verified IsSingleplayer function. Binaries are not redistributed.
"""
import argparse
import hashlib
import json
...
template = (root / 'tests/gmoddatapack_resolver_harness.cpp').read_text(encoding='utf-8')
...
with tempfile.TemporaryDirectory(prefix='datapack-resolver-', dir=args.work_dir) as directory:
    ...
    modes = ['nominal', 'corrupt-signature', 'duplicate-signature', 'corrupt-type-name',
             'duplicate-type-name', 'duplicate-name-ref', 'duplicate-vtable-ref',
             'zero-typeinfo-name', 'wrong-slot', 'oversized-data-segment',
             'no-read', 'non-executable', 'two-executable-segments', 'null-module']
    for fixture in fixtures:
        print('Fixture SHA-256: ' + fixture['sha256'], flush=True)
        for mode in modes:
            subprocess.run([str(binary), fixture['file'], fixture['isp'], mode],
